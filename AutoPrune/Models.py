...
from AutoPrune.Dropouts import *
...
class almostOneHotNN(nn.Module):
    def __init__(self, input_size, num_rules, num_neurons, initial_weight_val=0.2, final_activation=None,
        ...
        self.weights_dropout = RegularFeatureDropout(dropout_perc, num_neurons, burn_in)

        self.final_activation = None
        if final_activation is not None:
            self.final_activation = final_activation()

    def forward(self, x, epoch=None, loss=None):
        ...
        weights_masks = self.weights_dropout(self.hidden_rules, epoch, loss)
...
class almostOneHotNNv2(nn.Module):
    def __init__(self, input_size, num_rules, dropout_module, initial_weight_val=0.2, final_activation=None,
        ...
        super(almostOneHotNNv2, self).__init__()
        self.num_rules = num_rules
        self.out_weight_constraint = out_weight_constraint
        self.rule_activation = rule_activation()
        ...
        self.weights_dropout = dropout_module
...
class ClusterOneHotNN(nn.Module):
    def __init__(self, input_size, n_clusters, num_rules, num_neurons, initial_weight_val=0.2, final_activation=None,
        ...
        for cluster in range(n_clusters):
            ...
            weights_dropout = RegularFeatureDropout(dropout_perc, num_neurons, burn_in)
...
class ClusterOneHotNNv2(nn.Module):
    def __init__(self, input_size, n_clusters, num_rules, dropout_module, initial_weight_val=0.2, final_activation=None,
                 rule_bias=False, force_positive_out_init=True, rule_activation=None,
                 out_bias=False, out_weight_constraint=None, dropout_perc=0.5, burn_in=10, dtype=torch.float64):

        super(ClusterOneHotNNv2, self).__init__()
        self.n_clusters = n_clusters
        self.num_rules = num_rules
        self.out_weight_constraint = out_weight_constraint
        self.rule_activation = rule_activation()
        ...
        for cluster in range(n_clusters):
            ...
            weights_dropout = copy.deepcopy(dropout_module)
            pipeline = nn.ModuleList([hidden_rules, rule_weight, dropout, weights_dropout])
            self.model.append(pipeline)
    ...
    def forward(self, x, epoch=None, loss=None):
        ...
        for cluster in range(self.n_clusters):
            ...
            cluster_mask = index.unsqueeze(-1).repeat_interleave(inputs.size(-1), -1)
            hidden_rules, rule_weight, dropout, weights_dropout = self.model[cluster]
            weights_masks = weights_dropout(hidden_rules, epoch, loss)
